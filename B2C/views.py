...
def _search(keyword=None, cate_id=None, price_level=0, discount_level=0):
    ...
    items = Item.query.filter(Item.item_name.like(''.join(['%',keyword,'%'])),
                              ...
                              Item.price < price_map[
                                  price_level]['upper_bound'],
                              Item.discount >= discount_map[
                                  discount_level]['lower_bound'],
...
def item_search():
    ...
    if request.method == 'POST':
        ...
        items = _search(request.form['keyword'], cate_id, int(
            request.form['price_level']), int(request.form['discount_level']))
...
def do_find_password():
    ...
    db.session.commit()
    send_email(email_address, "Retrive Password",
...
def send_email(to_email_address, subject, text):
    ...
    try:
        server.sendmail(
            'b1123710410@ssmail.hit.edu.cn', [to_email_address], msg.as_string())
    finally:
        server.quit()
...
def admin_login():
    ...
    if request.method == 'POST':
        admin = Admin.query.filter_by(
            admin_name=request.form['account']).first()
        if admin is None:
    ...
    if request.method == 'POST':
        if 'remove' in request.form:
            if 'cate' in request.form:
                a = TopDirectory.query.filter_by(
                    id=request.form['cate']).first()
                db.session.delete(a)
                db.session.commit()
                return redirect(url_for('edit_dir'))
...
def add_item():
    ...
    a = Item(request.form['name'], request.form['description'], request.form['price'], request.form[
             'discount'], request.form['storage'], request.form['cate_id'], image=path)
...
def add_comment(id):
    if request.method == 'POST':
        date = datetime.now()
        comment = Comment(
            request.form['like'], '', request.form['desc'], date, id, g.user.id)
...
def collect(id):
    assert request.method == 'GET'
    r = db.session.execute(user_collection_re.insert(), {
                           'user_id': g.user.id, 'item_id': id, 'date': datetime.now()})
    if r.is_insert:
        db.session.commit()
